...
随着以WorkBuddy、豆包工作等智能体工具的兴起，AI的编程能力越来越好，编写数据大屏越来越简单。简单到就一句话：

> 访问某某地址（即WebAPI），编写一个数据大屏，每隔10秒钟更新一次。

如果是在原来的任务下增加这句指令，AI肯定能理解。如果是一个全新的任务，最好同时附上收集数据的交互网页，指令如下：

> 这个网页收集的数据，可以通过某某地址（即WebAPI）访问。根据这个数据，编写一个数据大屏，每隔10秒钟更新一次。
